// Old Swedish translation by Benjamin Horn · https://benjaminhorn.io/
// Old Dutch translation by Osman Temiz · u/MANllAC
// Old Polish translation by Mateusz K · u/DiVine92
// Old Russian translation by OemDef · u/OemDef
// Old Simplified chinese translation by Shuhuai Cao · github.com/csh980717
// Old Slovak translation by Roman Bartík
// Old German translation by Bernhard Wittmann · https://bernhardwittmann.com
// Old Italian translation by Trazalca · github.com/Trazalca
// Old Spanish translation by José Alberto · github.com/joatb
// Brazilian translation by Adilson Santos · github.com/adilsonfsantos

// {
// 	fr: '',
// 	sv: '',
// 	nl: '',
// 	pl: '',
// 	pt_BR: '',
// 	ru: '',
// 	zh_CN: '',
// 	sk: '',
// 	de: '',
// 	it: '',
// 	es: '',
// }

const dict = {
	'New Tab': {
		fr: 'Nouvel onglet',
		sv: 'Ny flik',
		nl: 'Nieuw blad',
		pl: 'Nowa Karta',
		pt_BR: 'Nova Aba',
		ru: 'Новая вкладка',
		zh_CN: '新标签页',
		sk: 'Nová karta',
		de: 'Neuer Tab',
		it: 'Nuova scheda',
		es: 'Nueva Pestaña',
	},
	Consider: {
		fr: "N'hésitez pas",
		sv: 'Överväga',
		nl: 'Overwegen',
		pl: 'Rozważ',
		pt_BR: 'Considerar',
		ru: 'Рассмотреть возможность',
		zh_CN: '您可以考虑',
		sk: 'Zvážte',
		de: 'Erwägen',
		it: 'Considera di',
		es: 'Considerar',
	},
	donating: {
		fr: 'faire un don',
		sv: 'donera',
		nl: 'doneren',
		pl: 'przekazanie darowizny',
		pt_BR: 'doar',
		ru: 'пожертвование',
		zh_CN: '向我们经营',
		sk: 'darovanie',
		de: 'spenden',
		it: 'fare una donazione',
		es: 'hacer una donación',
	},
	'if you love Bonjourr': {
		fr: 'si vous aimez Bonjourr',
		sv: 'om du älskar Bonjourr',
		nl: 'Als je van Bonjourr houdt',
		pl: 'jeśli kochasz Bonjourr',
		pt_BR: 'se você ama o Bonjourr',
		ru: 'если вы любите Bonjourr',
		zh_CN: '来支持卓悦',
		sk: 'Ak miluješ Bonjourra',
		de: 'wenn du Bonjourr liebst',
		it: 'se ami Bonjourr',
		es: 'si amas Bonjourr',
	},
	'Love using Bonjourr? Consider giving us a review or donating, that would help a lot! 😇': {
		fr: 'Vous adorez utiliser Bonjourr ? Pensez à nous noter ou faire un don, ça serait super ! 😇',
		sv: 'Du älskar att använda Bonjourr? Överväg att ge oss en recension, det skulle hjälpa mycket!',
		nl: 'Ben je dol op het gebruik van Bonjourr? Overweeg om ons een recensie te geven, dat zou enorm helpen! 😇',
		pl: 'Uwielbiasz korzystać z Bonjourr? Rozważ napisanie nam recenzji, która bardzo by nam pomogła! 😇',
		pt_BR: 'Gosta de usar o Bonjourr? Considere nos avaliar, isso ajudaria muito! 😇',
		ru: 'Вам нравится использовать Bonjourr? Подумайте о том, чтобы дать нам обзор, это очень поможет! 😇',
		zh_CN: '你喜欢使用 Bonjourr？考虑给我们一个评论,这会很有帮助！😇',
		sk: 'Máte radi Bonjourr? Skúste nám dať recenziu, ktorá by vám veľmi pomohla!',
		de: 'Du liebst es, Bonjourr zu benutzen? Erwäge, uns eine Bewertung zu geben, das würde sehr helfen! 😇',
		it: 'Ti piace usare Bonjourr? Considera di darci una recensione, sarebbe di grande aiuto! 😇',
		es: '¿Te gusta Bonjourr? ¡Considera escribir una reseña, nos ayudaría mucho! 😇',
	},
	Review: {
		fr: 'Noter',
		sv: 'Recension',
		nl: 'Beoordeling',
		pl: 'Przejrzeć',
		pt_BR: 'Análise',
		ru: 'Рассмотрение',
		zh_CN: '审查',
		sk: 'Preskúmanie',
		de: 'Rezension',
		it: 'Recensione',
		es: 'Revisar',
	},
	Donate: {
		fr: 'Faire un don',
		sv: 'Donera',
		nl: 'Doneer',
		pl: 'Podarować',
		pt_BR: 'Corça',
		ru: 'Пожертвовать',
		zh_CN: '捐',
		sk: 'Darovať',
		de: 'Spenden',
		it: 'Donare',
		es: 'Donar',
	},
	Cancel: {
		fr: 'Annuler',
		sv: 'Avbryt',
		nl: 'Annuleren',
		pl: 'Anulować',
		pt_BR: 'Cancelar',
		ru: 'Отмена',
		zh_CN: '取消',
		sk: 'Zrušiť',
		de: 'Abbrechen',
		it: 'Annulla',
		es: 'Cancelar',
	},
	Delete: {
		fr: 'Supprimer',
		sv: 'Radera',
		nl: 'Verwijderen',
		pl: 'Kasować',
		pt_BR: 'Deletar',
		ru: 'Удалить',
		zh_CN: '删除快速链接',
		sk: 'Odstrániť',
		de: 'Löschen',
		it: 'Elimina',
		es: 'Eliminar',
	},
	'Apply changes': {
		fr: 'Appliquer',
		sv: 'Applicera förändringar',
		nl: 'Wijzigingen toepassen',
		pl: 'Zatwierdź zmiany',
		pt_BR: 'Aplicar mudanças',
		ru: 'Применить изменения',
		zh_CN: '保存更改',
		sk: 'Aplikovať zmeny',
		de: 'Änderungen übernehmen',
		it: 'Applicazioni modifiche',
		es: 'Aplicar cambios',
	},
	Monday: {
		fr: 'Lundi',
		sv: 'Måndag',
		nl: 'Maandag',
		pl: 'Poniedziałek',
		pt_BR: 'Segunda',
		ru: 'Понедельник',
		zh_CN: '蒙',
		sk: 'Pondelok',
		de: 'Montag',
		it: 'Lunedì',
		es: 'Lunes',
	},
	Tuesday: {
		fr: 'Mardi',
		sv: 'Tisdag',
		nl: 'Dinsdag',
		pl: 'Wtorek',
		pt_BR: 'Terça',
		ru: 'Вторник',
		zh_CN: '星期二',
		sk: 'Utorok',
		de: 'Dienstag',
		it: 'Martedì',
		es: 'Martes',
	},
	Wednesday: {
		fr: 'Mercredi',
		sv: 'Onsdag',
		nl: 'Woensdag',
		pl: 'Środa',
		pt_BR: 'Quarta',
		ru: 'Среда',
		zh_CN: '星期三',
		sk: 'Streda',
		de: 'Mittwoch',
		it: 'Mercoledì',
		es: 'Miércoles',
	},
	Thursday: {
		fr: 'Jeudi',
		sv: 'Torsdag',
		nl: 'Donderdag',
		pl: 'Czwartek',
		pt_BR: 'Quinta',
		ru: 'Четверг',
		zh_CN: '你',
		sk: 'Štvrtok',
		de: 'Donnerstag',
		it: 'Giovedì',
		es: 'Jueves',
	},
	Friday: {
		fr: 'Vendredi',
		sv: 'Fredag',
		nl: 'Vrijdag',
		pl: 'Piątek',
		pt_BR: 'Sexta',
		ru: 'Пятница',
		zh_CN: '关于',
		sk: 'Piatok',
		de: 'Freitag',
		it: 'Venerdì',
		es: 'Viernes',
	},
	Saturday: {
		fr: 'Samedi',
		sv: 'Lördag',
		nl: 'Zaterdag',
		pl: 'Sobota',
		pt_BR: 'Sábado',
		ru: 'Суббота',
		zh_CN: '星期六',
		sk: 'Sobota',
		de: 'Samstag',
		it: 'Sabato',
		es: 'Sábado',
	},
	Sunday: {
		fr: 'Dimanche',
		sv: 'Söndag',
		nl: 'zondag',
		pl: 'Niedziela',
		pt_BR: 'Domingo',
		ru: 'Воскресенье',
		zh_CN: '星期日',
		sk: 'Nedeľa',
		de: 'Sonntag',
		it: 'Domenica',
		es: 'Domingo',
	},
	January: {
		fr: 'Janvier',
		sv: 'Januari',
		nl: 'Januari',
		pl: 'Styczeń',
		pt_BR: 'Janeiro',
		ru: 'января',
		zh_CN: '1月',
		sk: 'Január',
		de: 'Januar',
		it: 'Gennaio',
		es: 'Enero',
	},
	February: {
		fr: 'Février',
		sv: 'Februari',
		nl: 'februari',
		pl: 'Luty',
		pt_BR: 'Fevereiro',
		ru: 'февраля',
		zh_CN: '2月',
		sk: 'Február',
		de: 'Februar',
		it: 'Febbraio',
		es: 'Febrero',
	},
	March: {
		fr: 'Mars',
		sv: 'Mars',
		nl: 'Maart',
		pl: 'Marzec',
		pt_BR: 'Março',
		ru: 'марта',
		zh_CN: '三月',
		sk: 'Marec',
		de: 'März',
		it: 'Marzo',
		es: 'Marzo',
	},
	April: {
		fr: 'Avril',
		sv: 'April',
		nl: 'April',
		pl: 'Kwiecień',
		pt_BR: 'Abril',
		ru: 'апреля',
		zh_CN: '4月',
		sk: 'Apríl',
		de: 'April',
		it: 'Aprile',
		es: 'Abril',
	},
	May: {
		fr: 'Mai',
		sv: 'Maj',
		nl: 'Mei',
		pl: 'Maj',
		pt_BR: 'Maio',
		ru: 'мая',
		zh_CN: '5月',
		sk: 'Máj',
		de: 'Mai',
		it: 'Maggio',
		es: 'Mayonesa',
	},
	June: {
		fr: 'Juin',
		sv: 'Juni',
		nl: 'Juni',
		pl: 'Czerwiec',
		pt_BR: 'Junho',
		ru: 'июня',
		zh_CN: '6月',
		sk: 'Jún',
		de: 'Juni',
		it: 'Giugno',
		es: 'Junio',
	},
	July: {
		fr: 'Juillet',
		sv: 'Juli',
		nl: 'Juli',
		pl: 'Lipiec',
		pt_BR: 'Julho',
		ru: 'июля',
		zh_CN: '7月',
		sk: 'Júl',
		de: 'Juli',
		it: 'Luglio',
		es: 'Julio',
	},
	August: {
		fr: 'Août',
		sv: 'Augusti',
		nl: 'Augustus',
		pl: 'Sierpień',
		pt_BR: 'Agosto',
		ru: 'августа',
		zh_CN: '8月',
		sk: 'August',
		de: 'August',
		it: 'Agosto',
		es: 'Agosto',
	},
	September: {
		fr: 'Septembre',
		sv: 'September',
		nl: 'September',
		pl: 'Wrzesień',
		pt_BR: 'Setembro',
		ru: 'сентября',
		zh_CN: '9月',
		sk: 'Spetember',
		de: 'September',
		it: 'Settembre',
		es: 'Septiembre',
	},
	October: {
		fr: 'Octobre',
		sv: 'Oktober',
		nl: 'oktober',
		pl: 'Październik',
		pt_BR: 'Outubro',
		ru: 'октября',
		zh_CN: '10月',
		sk: 'Október',
		de: 'Oktober',
		it: 'Ottobre',
		es: 'Octubre',
	},
	November: {
		fr: 'Novembre',
		sv: 'November',
		nl: 'november',
		pl: 'Listopad',
		pt_BR: 'Novembro',
		ru: 'ноября',
		zh_CN: '11月',
		sk: 'November',
		de: 'November',
		it: 'Novembre',
		es: 'Noviembre',
	},
	December: {
		fr: 'Décembre',
		sv: 'December',
		nl: 'December',
		pl: 'Grudzień',
		pt_BR: 'Dezembro',
		ru: 'декабря',
		zh_CN: '12月',
		sk: 'December',
		de: 'Dezember',
		it: 'Dicembre',
		es: 'Diciembre',
	},
	'Good morning': {
		fr: 'Bonjour',
		sv: 'Godmorgon',
		nl: 'Goedemorgen',
		pl: 'Dzień dobry',
		pt_BR: 'Bom Dia',
		ru: 'Доброе утро!',
		zh_CN: '早上好',
		sk: 'Dobré ráno',
		de: 'Guten Morgen',
		it: 'Buongiorno',
		es: 'Buenos días',
	},
	'Good afternoon': {
		fr: 'Bonjour',
		sv: 'Goddag',
		nl: 'Goedemiddag',
		pl: 'Miłego popołudii',
		pt_BR: 'Boa tarde',
		ru: 'Добрый день!',
		zh_CN: '下午好',
		sk: 'Dobrý deň',
		de: 'Guten Nachmittag',
		it: 'Buon pomeriggio',
		es: 'Buenas tardes',
	},
	'Good evening': {
		fr: 'Bonsoir',
		sv: 'Gud kväll',
		nl: 'Goedenavond',
		pl: 'Dobry wieczór',
		pt_BR: 'Boa noite',
		ru: 'Добрый вечер!',
		zh_CN: '晚上好',
		sk: 'Dobrý večer',
		de: 'Guten Abend',
		it: 'Buona sera',
		es: 'Buenas noches',
	},
	'Good night': {
		fr: 'Bonne nuit',
		sv: 'Gud natt',
		nl: 'Goedennacht',
		pl: 'Dobrej nocy',
		pt_BR: 'Boa noite',
		ru: 'Доброй ночи!',
		zh_CN: '晚安',
		sk: 'Dobrú noc',
		de: 'Gute Nacht',
		it: 'Buonanotte',
		es: 'Buenas noches',
	},
	'It is currently': {
		fr: 'Il fait actuellement',
		sv: 'Det är bara nu',
		nl: 'Het is nu',
		pl: 'Jesteśmy obecnie',
		pt_BR: 'Atualmente faz',
		ru: 'Сейчас',
		zh_CN: '现在气温',
		sk: 'Momentálne je',
		de: 'Es ist derzeit',
		it: 'Ci sono',
		es: 'Actualmente está',
	},
	'with a high of': {
		fr: 'avec une maximale de',
		sv: 'med högsta',
		nl: 'Met een grote',
		pl: 'z maksymalnie',
		pt_BR: 'um máximo é de',
		ru: 'максимальная температура воздуха сегодня',
		zh_CN: '最高温度',
		sk: 'z maximálnych',
		de: 'mit maximal',
		it: 'con una massima di',
		es: 'con un máximo de',
	},
	today: {
		fr: "aujourd'hui",
		sv: 'idag',
		nl: 'vandaag',
		pl: 'dzisiaj',
		pt_BR: 'hoje',
		ru: '',
		zh_CN: ', 以上是今日天气',
		sk: 'dnes',
		de: 'heute',
		it: 'oggi',
		es: 'hoy',
	},
	tomorrow: {
		fr: 'demain',
		sv: 'jutri',
		nl: 'morgen',
		pl: 'jutro',
		pt_BR: 'amanhã',
		ru: '',
		zh_CN: ', 以上是明天的天气',
		sk: 'zajtra',
		de: 'morgen',
		it: 'domani',
		es: 'mañana',
	},
	Title: {
		fr: 'Titre',
		sv: 'Titel',
		nl: 'Titel',
		pl: 'Tytuł',
		pt_BR: 'Título',
		ru: 'Имя',
		zh_CN: '标题',
		sk: 'Názov',
		de: 'Titel',
		it: 'Titolo',
		es: 'Título',
	},
	'on Unsplash': {
		fr: 'sur Unsplash',
		sv: 'på Unsplash',
		nl: 'op Unsplash',
		pl: 'na Unsplash',
		pt_BR: 'sem Unsplash',
		ru: 'на Unsplash',
		zh_CN: '在 Unsplash',
		sk: 'na Unsplash',
		de: 'auf Unsplash',
		it: 'su Unsplash',
		es: 'en Unsplash',
	},
	'Icon must be < 8kB': {
		fr: "L'icone doit etre < 8kB",
		sv: 'Ikonen måste vara <8kB',
		nl: 'Icoon moet < 8kB zijn',
		pl: 'Ikona musi być < 8kB',
		pt_BR: 'O ícone deve ser <8kB',
		ru: 'Значок должен быть <8 КБ',
		zh_CN: '图标必须< 8kB',
		sk: 'Ikona musí byť <8 kB',
		de: 'Symbol muss < 8kB sein',
		it: "L'icona deve essere < 8 kB",
		es: 'El icono debe ser <8 kB',
	},
	GENERAL: {
		fr: 'Général',
		sv: 'Allmän',
		nl: 'Algemeen',
		pl: 'Ogólne',
		pt_BR: 'Geral',
		ru: 'Общий',
		zh_CN: '通用',
		sk: 'Všeobecné',
		de: 'Allgemein',
		it: 'Generale',
		es: 'General',
	},
	'Show all settings': {
		fr: 'Afficher tous les réglages',
		sv: 'Visa alla inställningar',
		nl: 'Toon alle instellingen',
		pl: 'Pokaż wszystkie ustawienia',
		pt_BR: 'Mostrar todas configurações',
		ru: 'Показать все настройки',
		zh_CN: '显示所有设置',
		sk: 'Zobraziť všetky nastavenia',
		de: 'Alle Einstellungen anzeigen',
		it: 'Mostra tutte le impostazioni',
		es: 'Mostrar todos los ajustes',
	},
	'QUICK LINKS': {
		fr: 'Liens favoris',
		sv: 'Snabblänkar',
		nl: 'Snelle-links',
		pl: 'Ulubione Witryny',
		pt_BR: 'LINKS RÁPIDOS',
		ru: 'Закладки',
		zh_CN: '快速链接',
		sk: 'RÝCHLA NAVIGÁCIA',
		de: 'Schnelllinks',
		it: 'Collegamenti rapidi',
		es: 'Enlaces favoritos',
	},
	'More info': {
		fr: "Plus d'info",
		sv: 'Mer information',
		nl: 'Meer informatie',
		pl: 'Więcej informacji',
		pt_BR: 'Mais informação',
		ru: 'Больше информации',
		zh_CN: '更多信息',
		sk: 'Viac informácií',
		de: 'Mehr Info',
		it: 'Ulteriori informazioni',
		es: 'Más información',
	},
	"These are Bonjourr's bookmarks. Edit your Quick Links by right clicking on their icon.": {
		fr: "Ce sont les favoris de Bonjourr. Éditez un lien favori en faisant un clic droit sur l'icône.",
		sv: 'Det här är Bonjourrs bokmärken. Redigera dina snabblänkar genom att högerklicka på deras ikon.',
		nl: 'Dit zijn de bladwijzers van Bonjourr. Bewerk uw snelkoppelingen door met de rechtermuisknop op hun pictogram te klikken.',
		pl: 'To są zakładki Bonjourr. Edytuj swoje szybkie linki, klikając prawym przyciskiem myszy ich ikonę.',
		pt_BR: 'Esses são os favoritos do Bonjourr. Altere seus links rápidos clicando com o botão direito do mouse em seu ícone.',
		ru: 'Это закладки Bonjourr. Отредактируйте быстрые ссылки, щелкнув их значок правой кнопкой мыши.',
		zh_CN: '这些是 Bonjourr 的书签。通过右键单击它们的图标来编辑您的快速链接。',
		sk: 'Toto sú záložky Bonjourru. Upravte svoje rýchle odkazy kliknutím pravým tlačidlom na ich ikonu.',
		de: 'Dies sind Bonjourrs Lesezeichen. Bearbeiten Sie Ihre Quick Links, indem Sie mit der rechten Maustaste auf das Symbol klicken.',
		it: 'Questi sono i segnalibri di Bonjourr. Modifica i tuoi collegamenti rapidi facendo clic con il pulsante destro del mouse sulla loro icona.',
		es: 'Estos son los marcadores de Bonjourr. Edite sus enlaces rápidos haciendo clic con el botón derecho en su icono.',
	},
	'Type the collection id from its URL, add more by separating them with a comma.': {
		fr: "Tapez l'ID de l'URL de la collection. Ajoutez en plusieurs en les séparant par une virgule.",
		sv: 'Skriv samlings id från dess URL. Lägg till mer genom att separera dem med ett komma.',
		nl: 'Typ de collectie ID van de URL. Voeg meer toe door ze te scheiden met een komma.',
		pl: 'Wpisz identyfikator kolekcji z adresu URL. Dodaj więcej oddzielając je przecinkiem.',
		pt_BR: 'Digite a identificação da coleção a partir da URL. Local mais separando-os com uma vírgula',
		ru: 'Введите идентификатор коллекции из ее URL-адреса. Добавьте другие, разделив их запятыми.',
		zh_CN: '从其 URL 中输入集合 ID。通过用逗号分隔它们来添加更多。',
		sk: 'Zadajte ID zbierky z jej adresy URL. Pridajte ďalšie ich oddelením čiarkou.',
		de: 'Geben Sie die Sammlungs ID aus ihrer URL ein. Fügen Sie weitere hinzu, indem Sie sie durch ein Komma trennen.',
		it: "Digita l'ID della raccolta dal relativo URL. Aggiungine altri separandoli con una virgola.",
		es: 'Escriba el ID de la colección de su URL. Agregue más separándolos con una coma.',
	},
	Name: {
		fr: 'Nom',
		sv: 'Namn',
		nl: 'Naam',
		pl: 'Nazwa',
		pt_BR: 'Nome',
		ru: 'название',
		zh_CN: '名称',
		sk: 'názov',
		de: 'Name',
		it: 'nome',
		es: 'Nombre',
	},
	Add: {
		fr: 'Ajouter',
		sv: 'Lägg till',
		nl: 'Voeg teen',
		pl: 'Dodaj',
		pt_BR: 'Adicionar',
		ru: 'Добавить',
		zh_CN: '添加',
		sk: 'Pridať',
		de: 'Hinzufügen',
		it: 'Aggiungere',
		es: 'Añadir',
	},
	'Open in new tab by default': {
		fr: 'Ouvrir dans un nouvel onglet par défaut',
		zh_CN: '默认在新标签页打开链接',
		sv: 'Öppna i ny flik som standard',
		nl: 'Standaard openen in nieuw tabblad',
		pl: 'Otwieraj domyślnie w nowej wersji',
		pt_BR: 'Abrir em nova aba por padrão',
		ru: 'По умолчанию открывать в новой вкладке',
		sk: 'Predvolene najmenej v novej karte',
		de: 'Standardmäßig in einem neuen Tab öffnen',
		it: 'Apri in una nuova scheda come predefinita',
		es: 'Abrir en una nueva pestaña por defecto',
	},
	'Links per row': {
		fr: 'Liens par ligne',
		sv: 'Länkar per rad',
		nl: 'Links per rij',
		pl: 'Linki na wiersz',
		pt_BR: 'Links por linha',
		ru: 'Ссылки в строке',
		zh_CN: '每行链接',
		sk: 'Odkazy na riadok',
		de: 'Links pro Zeile',
		it: 'Link per riga',
		es: 'Enlaces por fila',
	},
	VISUALS: {
		fr: 'Affichage',
		sv: 'Visuella',
		nl: 'Vormgeving',
		pl: 'Wygląd',
		pt_BR: 'VISUAIS',
		ru: 'Внешний вид',
		zh_CN: '外形',
		sk: 'VZHĽAD',
		de: 'Bilder',
		it: 'Visualizzazione',
		es: 'Visualización',
	},
	'Background type': {
		fr: "Type d'arrière-plan",
		sv: 'Bakgrundstyp',
		nl: 'Achtergrondtype',
		pl: 'Rodzaj tła',
		pt_BR: 'Tipo de fundo',
		ru: 'Тип фона',
		zh_CN: '背景类型',
		sk: 'Pozadie',
		de: 'Hintergrundtyp',
		it: 'Tipo di sfondo',
		es: 'Tipo de fondo',
	},
	Default: {
		fr: 'Par défaut',
		sv: 'Standard',
		nl: 'Standaard',
		pl: 'Domyślne',
		pt_BR: 'Padrão',
		ru: 'Дефолт',
		zh_CN: '默认',
		sk: 'Predvolené',
		de: 'Standard',
		it: 'Predefinito',
		es: 'Por Defecto',
	},
	Dynamic: {
		fr: 'Dynamique',
		sv: 'Dynamisk',
		nl: 'Dynamisch',
		pl: 'Dynamiczne',
		pt_BR: 'Dinâmico',
		ru: 'Динамический',
		zh_CN: '动态',
		sk: 'Dynamický',
		de: 'Dynamisch',
		it: 'dinamico',
		es: 'Dinámico',
	},
	Custom: {
		fr: 'Personnalisé',
		sv: 'Beställnings',
		nl: 'Aangepast',
		pl: 'Użytkownika',
		pt_BR: 'Personalizado',
		ru: 'Обычай',
		zh_CN: '自定义',
		sk: 'Vlastné',
		de: 'Benutzerdefiniert',
		it: 'Personalizzato',
		es: 'Personalizado',
	},
	Frequency: {
		fr: 'Fréquence',
		sv: 'Frekvens',
		nl: 'Frequentie',
		pl: 'Częstotliwość',
		pt_BR: 'Frequência',
		ru: 'Частота',
		zh_CN: '切换背景',
		sk: 'Frekvencia',
		de: 'Häufigkeit',
		it: 'Frequenza',
		es: 'Frecuencia',
	},
	'Every tab': {
		fr: 'Chaque onglet',
		sv: 'Varje flik',
		nl: 'Elk tabblad',
		pl: 'Co zakładkę',
		pt_BR: 'A cada aba',
		ru: 'Каждая вкладка',
		zh_CN: '每次新建标签页',
		sk: 'Každá karta',
		de: 'Jedes-Tab',
		it: 'Ogni scheda',
		es: 'Cada Pestaña',
	},
	'Every hour': {
		fr: 'Chaque heure',
		sv: 'Varje timme',
		nl: 'Elk uur',
		pl: 'Wspólna godzina',
		pt_BR: 'A cada hora',
		ru: 'Каждый час',
		zh_CN: '旅',
		sk: 'Každú hodinu',
		de: 'Jede Stunde',
		it: 'Ogni ora',
		es: 'Cada hora',
	},
	'Every day': {
		fr: 'Chaque jour',
		sv: 'Varje dag',
		nl: 'Elke dag',
		pl: 'Codziennie',
		pt_BR: 'Todos os dias',
		ru: 'Каждый день',
		zh_CN: '每天',
		sk: 'Každý deň',
		de: 'Jeden-Tag',
		it: 'Ogni giorno',
		es: 'Cada día',
	},
	Pause: {
		fr: 'Pause',
		sv: 'Paus',
		nl: 'Pauze',
		pl: 'Pauza',
		pt_BR: 'Pausar',
		ru: 'Пауза',
		zh_CN: '不切换',
		sk: 'Pauza',
		de: 'Pause',
		it: 'Pausa',
		es: 'Pausa',
	},
	'Unsplash collection': {
		fr: 'Collection Unsplash',
		sv: 'Unsplash samling',
		nl: 'Unsplash verzameling',
		pl: 'Kolekcja Unsplash',
		pt_BR: 'Coleção do Unsplash',
		ru: 'Unsplash collection',
		zh_CN: 'Unsplash 收藏',
		sk: 'Unsplash collection',
		de: 'Unsplash Sammlung',
		it: 'Raccolta Unsplash',
		es: 'Colección Unsplash',
	},
	'Blur intensity': {
		fr: 'Intensité du flou',
		sv: 'Oskärpans intensitet',
		nl: 'Intensiviteit van het vervagingseffect',
		pl: 'Intensywnośc rozmycia',
		pt_BR: 'Intensidade do desfoque',
		ru: 'Степень размытия фона',
		zh_CN: '可疑度',
		sk: 'Intenzita rozmazania',
		de: 'Weichzeichner Stärke',
		it: 'Intensità della sfocatura',
		es: 'Intensidad de desenfoque',
	},
	Brightness: {
		fr: 'Luminosité',
		sv: 'Ljusstyrka',
		nl: 'Helderheid',
		pl: 'Jasność',
		pt_BR: 'Brilho',
		ru: 'Яркость',
		zh_CN: '重新',
		sk: 'Jas',
		de: 'Helligkeit',
		it: 'Luminosità',
		es: 'Brillo',
	},
	'Dark mode': {
		fr: 'Mode sombre',
		sv: 'Mörkt läge',
		nl: 'Nachtmodus',
		pl: 'Tryb ciemny',
		pt_BR: 'Modo noturno',
		ru: 'Тёмная тема',
		zh_CN: '示范模式',
		sk: 'Tmavý režim',
		de: 'Dunkelmodus',
		it: 'Modalità notturna',
		es: 'Modo Oscuro',
	},
	'At night': {
		fr: 'La nuit',
		sv: 'På natten',
		nl: 'nachten',
		pl: 'W nocy',
		pt_BR: 'A noite',
		ru: 'ночью',
		zh_CN: '只在夜间',
		sk: 'V noci',
		de: 'Nacht',
		it: 'Di notte',
		es: 'De noche',
	},
	System: {
		fr: 'Système',
		sv: 'Systemet',
		nl: 'Systeem',
		pl: 'Systemem',
		pt_BR: 'Seguir sistema',
		ru: 'система',
		zh_CN: '系统设置',
		sk: 'Systému',
		de: 'System',
		it: 'Sistema',
		es: 'Sistema',
	},
	Enabled: {
		fr: 'Activé',
		sv: 'Aktiverad',
		nl: 'Ingeschakeld',
		pl: 'Włączony',
		pt_BR: 'Ativado',
		ru: 'Включена',
		zh_CN: '开启',
		sk: 'Zapnuté',
		de: 'aktiviert',
		it: 'Abilitato',
		es: 'Activado',
	},
	Disabled: {
		fr: 'Désactivé',
		sv: 'Inaktiverad',
		nl: 'Uitgeschakeld',
		pl: 'Wyłączony',
		pt_BR: 'Desativado',
		ru: 'Выключена',
		zh_CN: '关闭',
		sk: 'Vypnuté',
		de: 'Deaktiviert',
		it: 'Disabilitato',
		es: 'Desactivado',
	},
	'Hide elements': {
		fr: 'Cacher des éléments',
		sv: 'Dölj element',
		nl: 'Verberg elementen',
		pl: 'Ukryj elementy',
		pt_BR: 'Esconder elementos',
		ru: 'Скрыть элементы',
		zh_CN: '隐藏元素',
		sk: 'Skryť prvky',
		de: 'Elemente ausblenden',
		it: 'Nascondi elementi',
		es: 'Esconder elementos',
	},
	Clock: {
		fr: 'Horloge',
		sv: 'Klocka',
		nl: 'Klok',
		pl: 'Zegar',
		pt_BR: 'Relógio',
		ru: 'Часы',
		zh_CN: '时钟',
		sk: 'Hodiny',
		de: 'Uhr',
		it: 'Orologio',
		es: 'Reloj',
	},
	'Clock face': {
		fr: "Style d'horloge",
		sv: 'Klocka',
		nl: 'Klok',
		pl: 'Zegar',
		pt_BR: 'Estilo do relógio',
		ru: 'Часы',
		zh_CN: '时钟',
		sk: 'Hodiny',
		de: 'Uhr',
		it: 'Orologio',
		es: 'Reloj',
	},
	None: {
		fr: 'Simple',
		sv: 'Ingen',
		nl: 'Geen',
		pl: 'Nic',
		pt_BR: 'Nenhum',
		ru: 'Никто',
		zh_CN: '没有任何',
		sk: 'Žiadne',
		de: 'Keiner',
		it: 'Nessuno',
		es: 'Ninguno',
	},
	Numbers: {
		fr: 'Chiffres',
		sv: 'Tal',
		nl: 'Nummers',
		pl: 'Liczby',
		pt_BR: 'Números',
		ru: 'Цифры',
		zh_CN: '数字',
		sk: 'Čísla',
		de: 'Zahlen',
		it: 'Numeri',
		es: 'Números',
	},
	Romans: {
		fr: 'Chiffres romains',
		sv: 'Romarna',
		nl: 'Romeinen',
		pl: 'Rzymianie',
		pt_BR: 'Algarismos Romanos',
		ru: 'Римляне',
		zh_CN: '罗马书',
		sk: 'Rimania',
		de: 'Römer',
		it: 'Romani',
		es: 'Romanos',
	},
	Graduations: {
		fr: 'Graduations',
		sv: 'Examen',
		nl: 'Afstuderen',
		pl: 'Ukończenia studiów',
		pt_BR: 'Marcações',
		ru: 'Выпускные',
		zh_CN: '毕业典礼',
		sk: 'Promócie',
		de: 'Abschlüsse',
		it: 'Lauree',
		es: 'Graduaciones',
	},
	Date: {
		fr: 'Date',
		sv: 'Datum',
		nl: 'Datum',
		pl: 'Data',
		pt_BR: 'Dados',
		ru: 'Дата',
		zh_CN: '日期',
		sk: 'Dátum',
		de: 'Datum',
		it: 'Data',
		es: 'Fecha',
	},
	Greetings: {
		fr: 'Bienvenue',
		sv: 'Hälsningar',
		nl: 'De groeten',
		pl: 'Pozdrowienia',
		pt_BR: 'Saudações',
		ru: 'Привет',
		zh_CN: '你好',
		sk: 'Pozdravom',
		de: 'Grüße',
		it: 'Saluti',
		es: 'Saludos',
	},
	Weather: {
		fr: 'Météo',
		sv: 'Väder',
		nl: 'Het weer',
		pl: 'Pogoda',
		pt_BR: 'Tempo',
		ru: 'Погода',
		zh_CN: '天气',
		sk: 'Počasie',
		de: 'Wetter',
		it: 'Tempo metereologico',
		es: 'Tiempo',
	},
	Icon: {
		fr: 'Icône',
		sv: 'Ikon',
		nl: 'Icoon',
		pl: 'Ikona',
		pt_BR: 'Ícone',
		ru: 'Значок',
		zh_CN: '图标',
		sk: 'Ikona',
		de: 'Symbol',
		it: 'Icona',
		es: 'Icono',
	},
	'Quick Links': {
		fr: 'Liens favoris',
		sv: 'Snabblänkar',
		nl: 'Snelle links',
		pl: 'Szybkie linki',
		pt_BR: 'Links Rápidos',
		ru: 'Быстрые ссылки',
		zh_CN: '快速链接',
		sk: 'Rýchle odkazy',
		de: 'Schnelllinks',
		it: 'Link veloci',
		es: 'Enlaces favoritos',
	},
	'Settings icon': {
		fr: 'Icône réglages',
		sv: 'Inställningsikon',
		nl: 'Instellingen icoon',
		pl: 'Ikona ustawień',
		pt_BR: 'Ícone de configuração',
		ru: 'Значок настроек',
		zh_CN: '设置图标',
		sk: 'Ikona nastavení',
		de: 'Einstellungssymbol',
		it: 'Icona Impostazioni',
		es: 'Icono de configuración',
	},
	'Upload background here': {
		fr: 'Ajoutez votre arrière-plan',
		sv: 'Ladda upp bakgrunden här',
		nl: 'Upload hier achtergrond',
		pl: 'Prześlij tło tutaj',
		pt_BR: 'Suba o seu background aqui',
		ru: 'Загрузить фон здесь',
		zh_CN: '上传背景',
		sk: 'Nahrať pozadie tu',
		de: 'Hier Hintergrund hochladen',
		it: 'Carica lo sfondo qui',
		es: 'Suba el fondo aquí',
	},
	'Analog clock': {
		fr: 'Horloge analogue',
		sv: 'Analog klocka',
		nl: 'Analoge klok',
		pl: 'Zegar analogowy',
		pt_BR: 'Relógio Analógico',
		ru: 'Аналоговые часы',
		zh_CN: '模拟时钟',
		sk: 'Analógové hodiny',
		de: 'Analoge Uhr',
		it: 'Orologio analogico',
		es: 'Reloj Analógico',
	},
	'Display Seconds': {
		fr: 'Afficher les secondes',
		sv: 'Visa sekunder',
		nl: 'Weergave seconden',
		pl: 'Wyświetl sekundy',
		pt_BR: 'Mostrar segundos',
		ru: 'Показывать секунды',
		zh_CN: '显示秒',
		sk: 'Zobraziť sekundy',
		de: 'Sekunden anzeigen',
		it: 'Visualizza secondi',
		es: 'Mostrar los segundos',
	},
	Automatic: {
		fr: 'Automatique',
		sv: 'Automatiskt',
		nl: 'Automatisch',
		pl: 'Automatycznie',
		pt_BR: 'Automático',
		ru: 'Автоматически',
		zh_CN: '自动设置',
		sk: 'Automatické',
		de: 'Automatisch',
		it: 'Automatico',
		es: 'Automático',
	},
	WEATHER: {
		fr: 'Météo',
		sv: 'Väder',
		nl: 'Weer',
		pl: 'Pogoda',
		pt_BR: 'TEMPO',
		ru: 'Погода',
		zh_CN: '天气',
		sk: 'POČASIE',
		de: 'Wetter',
		it: 'Meteo',
		es: 'Tiempo',
	},
	Geolocation: {
		fr: 'Géolocalisation',
		sv: 'Geolokalisering',
		nl: 'Locatie',
		pl: 'Geolokalizacja',
		pt_BR: 'Geolocalização',
		ru: 'Геолокация',
		zh_CN: '使用我的位置',
		sk: 'Geolokácia',
		de: 'Geolokalisierung',
		it: 'Geolocalizzazione',
		es: 'Geolocalización',
	},
	'City not found': {
		fr: 'Ville non trouvé',
		sv: 'Staden hittades inte',
		nl: 'Stad niet gevonden',
		pl: 'Zmień miasto',
		pt_BR: 'Cidade não encontrada',
		ru: 'Город не найден',
		zh_CN: '寻找城市',
		sk: 'Mesto nenájdené',
		de: 'Stadt nicht gefunden',
		it: 'Città non trovata',
		es: 'Ciudad no encontrada',
	},
	'Change city': {
		fr: 'Changer la ville',
		sv: 'Byt stad',
		nl: 'Veranderlocatie',
		pl: 'Zmień miasto',
		pt_BR: 'Mudar cidade',
		ru: 'Сменить город',
		zh_CN: '更改城市',
		sk: 'Zmeniť mesto',
		de: 'Stadt ändern',
		it: 'Cambia città',
		es: 'Cambiar ciudad',
	},
	'There was a problem': {
		fr: 'Il y a eu un problème',
		sv: 'Ett fel uppstod',
		nl: 'Er is een probleem',
		pl: 'Wyeliminuj problem',
		pt_BR: 'Houve um problema',
		ru: 'Возникла проблема',
		zh_CN: '出现错误',
		sk: 'Nastal problém',
		de: 'Ein Problem ist aufgetreten',
		it: "C'è stato un problema",
		es: 'Hubo un problema',
	},
	'Imperial units': {
		fr: 'Unités impériales',
		sv: 'Imperial enheter',
		nl: 'Empirische eenheden',
		pl: 'Jednostki Imperialne',
		pt_BR: 'Unidades imperiais',
		ru: 'Использовать Фаренгейты',
		zh_CN: '英制单位',
		sk: 'Imperiálne jednotky',
		de: 'Imperiale Maßeinheiten',
		it: 'Sistema Imperiale',
		es: 'Sistema imperial',
	},
	'Search bar': {
		fr: 'Barre de recherche',
		sv: 'Sökruta',
		nl: 'Zoek balk',
		pl: 'Pasek wyszukiwania',
		pt_BR: 'Barra de pesquisa',
		ru: 'Строка поиска',
		zh_CN: '搜索框',
		sk: 'VYHĽADÁVACÍ PANEL',
		de: 'Suchleiste',
		it: 'Barra di ricerca',
		es: 'Barra de búsqueda',
	},
	'Search engine': {
		fr: 'Moteur de recherche',
		sv: 'Sökmotor',
		nl: 'Zoek motor',
		pl: 'Wyszukiwarka',
		pt_BR: 'Buscadores',
		ru: 'Поисковая система',
		zh_CN: '搜索引擎',
		sk: 'Vyhľadávač',
		de: 'Suchmaschine',
		it: 'Motore di ricerca',
		es: 'Motor de búsqueda',
	},
	Enable: {
		fr: 'Activer',
		sv: 'Aktivera',
		nl: 'Activeren',
		pl: 'aktywować',
		pt_BR: 'Ativar',
		ru: 'позволять',
		zh_CN: '允许',
		sk: 'Aktivovať',
		de: 'Aktivieren',
		it: 'Abilità',
		es: 'Activar',
	},
	'Open in new tab': {
		fr: 'Ouvrir dans un nouvel onglet',
		zh_CN: '在新标签页打开链接',
		sv: 'Öppna i ny flik',
		nl: 'Openen in nieuw tabblad',
		pl: 'Otwieraj w nowej nowej',
		pt_BR: 'Abrir em nova aba',
		ru: 'Открыть в новой вкладке',
		sk: 'Otvoriť v novej karte',
		de: 'In einem neuen Tab öffnen',
		it: 'Apri in una nuova scheda',
		es: 'Abrir en una nueva pestaña',
	},
	'URL request': {
		fr: 'URL de requête',
		sv: 'URL begäran',
		nl: 'URL verzoek',
		pl: 'Żądanie adresu URL',
		pt_BR: 'Solicitação de URL',
		ru: 'URL запрос',
		zh_CN: '网址请求',
		sk: 'Žiadosť o adresu URL',
		de: 'URL Anfrage',
		it: 'Richiesta URL',
		es: 'Solicitud de URL',
	},
	'%s Not found': {
		fr: '%s Pas trouvé',
		sv: '%s Hittades inte',
		nl: '%s Niet gevonden',
		pl: 'Nie znaleziono %s',
		pt_BR: '% s não encontrado',
		ru: '% s не найдено',
		zh_CN: '%s 未找到',
		sk: '%s nebol nájdený',
		de: '%s nicht gefunden',
		it: '%s non trovato',
		es: '% s no encontrado',
	},
	'Search query: %s': {
		fr: 'Requête: %s',
		sv: 'Sökfråga: %s',
		nl: 'Zoekopdracht: %s',
		pl: 'Wyszukiwane hasło: %s',
		pt_BR: 'Consulta de pesquisa:% s',
		ru: 'Поисковый запрос:% s',
		zh_CN: '搜索查询：%s',
		sk: 'Vyhľadávací dopyt: %s',
		de: 'Suchanfrage: %s',
		it: 'Ricerca di ricerca: %s',
		es: 'Consulta de búsqueda:% s',
	},
	'Time & Date': {
		fr: 'Date & heure',
		sv: 'Tid och datum',
		nl: 'Tijd datum',
		pl: 'Data i godzina',
		pt_BR: 'Data & Hora',
		ru: 'Время и дата',
		zh_CN: '时间和日期',
		sk: 'Čas a dátum',
		de: 'Zeit Datum',
		it: "Ora dell'appuntamento",
		es: 'Fecha & hora',
	},
	'CUSTOM FONT': {
		fr: 'Police personnalisée',
		sv: 'Anpassad FONT',
		nl: 'Aangepast LETTERTYPE',
		pl: 'Niestandardowa czcionka',
		pt_BR: 'FONTE PERSONALIZADA',
		ru: 'Пользовательский шрифт',
		zh_CN: '自定义字体',
		sk: 'Vlastné FONT',
		de: 'Benutzerdefinierte SCHRIFTART',
		it: 'CARATTERE personalizzato',
		es: 'Fuente personalizada',
	},
	'Custom Style': {
		fr: 'Style personnalisé',
		sv: 'Anpassad stil',
		nl: 'Aangepaste stijl',
		pl: 'Niestandardowy styl',
		pt_BR: 'Estilo personalizado',
		ru: 'Пользовательский стиль',
		zh_CN: '自定义样式',
		sk: 'Vlastný štýl',
		de: 'Benutzerdefinierter Stil',
		it: 'Stile personalizzato',
		es: 'Estilo personalizado',
	},
	'Type in your custom CSS': {
		fr: 'Entrez votre CSS personnalisé',
		sv: 'Skriv in din anpassade CSS',
		nl: 'Typ uw aangepaste CSS in',
		pl: 'Wpisz swój niestandardowy CSS',
		pt_BR: 'Insira seu CSS personalizado',
		ru: 'Введите свой собственный CSS',
		zh_CN: '输入您的自定义 CSS',
		sk: 'Zadajte svoj vlastný CSS',
		de: 'Geben Sie Ihr benutzerdefiniertes CSS ein',
		it: 'Digita il tuo CSS personalizzato',
		es: 'Escribe aquí tu CSS personalizado',
	},
	'US Date Format': {
		fr: 'Date américaine',
		sv: 'US Date Format',
		nl: 'Amerikaanse datumnotatie',
		pl: 'Format daty w USA',
		pt_BR: 'Formato de dados dos EUA',
		ru: 'Формат даты США',
		zh_CN: '美国日期格式',
		sk: 'Formát dátumu v USA',
		de: 'US-Datumsformat',
		it: 'Formato data USA',
		es: 'Formato de fecha americano',
	},
	'12-Hour Time': {
		fr: 'Affichage 12h',
		sv: '12-timmars klocka',
		nl: '12 uur tijd',
		pl: 'Czas 12-godzinny',
		pt_BR: 'Formato de 12 horas',
		ru: '12-часовой формат времени',
		zh_CN: '12小时制',
		sk: '12-hodinový čas',
		de: '12-Stunden Uhrzeit',
		it: 'Sistema orario a 12 ore',
		es: 'Sistema horario de 12 horas',
	},
	'Time zone': {
		fr: 'Fuseau horaire',
		sv: 'Tidszon',
		nl: 'Tijdzone',
		pl: 'Strefa czasowa',
		pt_BR: 'Fuso Horário',
		ru: 'Часовой пояс',
		zh_CN: '时区',
		sk: 'Časové pásmo',
		de: 'Zeitzone',
		it: 'Fuso orario',
		es: 'Huso horario',
	},
	Language: {
		fr: 'Langue',
		sv: 'Välj språk',
		nl: 'Kies een taal',
		pl: 'Wybierz Język',
		pt_BR: 'Idioma',
		ru: 'Выберите язык',
		zh_CN: '选择语言',
		sk: 'Vybrať jazyk',
		de: 'Sprache',
		it: 'Lingua',
		es: 'Lenguaje o idioma',
	},
	Greeting: {
		fr: 'Bienvenue',
		sv: 'Hälsning',
		nl: 'Groet',
		pl: 'Powitanie',
		pt_BR: 'Saudação',
		ru: 'Приветствие',
		zh_CN: '问候',
		sk: 'S pozdravom',
		de: 'Gruß',
		it: 'Saluto',
		es: 'Saludo',
	},
	'Custom CSS': {
		fr: 'CSS personnalisé',
		sv: 'Anpassad CSS',
		nl: 'Aangepaste CSS',
		pl: 'Niestandardowe CSS',
		pt_BR: 'CSS Personalizado',
		ru: 'Пользовательские CSS',
		zh_CN: '自定义 CSS',
		sk: 'Vlastný CSS',
		de: 'Benutzerdefinierte CSS',
		it: 'CSS personalizzato',
		es: 'CSS personalizado',
	},
	'Custom Font': {
		fr: 'Police personnalisée',
		sv: 'Anpassat teckensnitt',
		nl: 'Aangepast lettertype',
		pl: 'Czcionka Niestandardowa',
		pt_BR: 'Fonte Personalizada',
		ru: 'Пользовательский шрифт',
		zh_CN: '自定义字体',
		sk: 'Vlastné písmo',
		de: 'Benutzerdefinierte Schriftart',
		it: 'Carattere personalizzato',
		es: 'Fuente personalizada',
	},
	'Font family': {
		fr: 'Police',
		sv: 'Typsnittsfamilj',
		nl: 'Lettertypefamilie',
		pl: 'Rodzina czcionek',
		pt_BR: 'Família de fontes',
		ru: 'Семейство шрифтов',
		zh_CN: '字体系列',
		sk: 'Rodina písem',
		de: 'Schriftfamilie',
		it: 'Famiglia di font',
		es: 'Fuente',
	},
	Weight: {
		fr: 'Épaisseur',
		sv: 'Vikt',
		nl: 'Gewicht',
		pl: 'Waga',
		pt_BR: 'Peso',
		ru: 'Масса',
		zh_CN: '重量',
		sk: 'Hmotnosť',
		de: 'Gewicht',
		it: 'Il peso',
		es: 'Peso',
	},
	Size: {
		fr: 'Taille',
		sv: 'Storlek',
		nl: 'Maat',
		pl: 'Rozmiar',
		pt_BR: 'Tamanho',
		ru: 'Размер',
		zh_CN: '尺寸',
		sk: 'Veľkosť',
		de: 'Größe',
		it: 'Taglia',
		es: 'Tamaño',
	},
	Settings: {
		fr: 'Réglages',
		sv: 'Inställningar',
		nl: 'Instellingen',
		pl: 'Ustawienia',
		pt_BR: 'Configurações',
		ru: 'Настройки',
		zh_CN: '设置',
		sk: 'Nastavenia',
		de: 'Einstellungen',
		it: 'Impostazioni',
		es: 'Configuración',
	},
	Import: {
		fr: 'Importer',
		sv: 'Importera',
		nl: 'Importeren',
		pl: 'Import',
		pt_BR: 'Importar',
		ru: 'Импортировать',
		zh_CN: '导入设置',
		sk: 'Importovať',
		de: 'Importieren',
		it: 'Importare',
		es: 'Importar',
	},
	Export: {
		fr: 'Exporter',
		sv: 'Exportera',
		nl: 'Exporteren',
		pl: 'Eksport',
		pt_BR: 'Exportar',
		ru: 'Экспорт',
		zh_CN: '导出设置',
		sk: 'Export',
		de: 'Export',
		it: 'Esportare',
		es: 'Exportar',
	},
	Copied: {
		fr: 'Copié',
		sv: 'Kopierat',
		nl: 'Gekopieerd',
		pl: 'Skopiowano',
		pt_BR: 'Copiado',
		ru: 'Скопировано',
		zh_CN: '已复制',
		sk: 'Skopírované',
		de: 'Kopiert',
		it: 'Copiato',
		es: 'Copiado',
	},
	'Import code': {
		fr: 'Importer le code',
		sv: 'Importera kod',
		nl: 'Code importeren',
		pl: 'Kod importu',
		pt_BR: 'Código importante',
		ru: 'Код импорта',
		zh_CN: '导入代码',
		sk: 'Importovať kód',
		de: 'Code importieren',
		it: 'Codice di importazione',
		es: 'Importar código',
	},
	'Error in import code': {
		fr: "Erreur dans le code d'importation",
		sv: 'IError i importkod',
		nl: 'Fout in importcode',
		pl: 'Błąd w kodzie importu',
		pt_BR: 'Erro ao importar código',
		ru: 'Ошибка в коде импорта',
		zh_CN: '导入代码错误',
		sk: 'Chyba v kóde importu',
		de: 'Fehler im Importcode',
		it: 'Errore nel codice di importazione',
		es: 'Error en el código de importación',
	},
	'Export code': {
		fr: "Code d'exportation",
		sv: 'exportkod',
		nl: 'exportcode',
		pl: 'kod eksportowy',
		pt_BR: 'Exportar código',
		ru: 'экспортный код',
		zh_CN: '导入代码',
		sk: 'vývozný kód',
		de: 'Exportcode',
		it: 'codice di esportazione',
		es: 'Exportar código',
	},
	'Click again to confirm': {
		fr: 'Cliquez encore une fois pour confirmer',
		sv: 'Klicka igen för att bekräfta',
		nl: 'Klik nogmaals om te bevestigen',
		pl: 'Kliknij ponownie, aby potwierdzić',
		pt_BR: 'Clique novamente para confirmar',
		ru: 'Щелкните еще раз для подтверждения',
		zh_CN: '再次点击确认',
		sk: 'Opätovným kliknutím potvrďte',
		de: 'Erneut klicken, um zu bestätigen',
		it: 'Fai nuovamente clic per confermare',
		es: 'Haga clic de nuevo para confirmar',
	},
	'Reset settings': {
		fr: 'Réinitialiser les réglages',
		sv: 'Återställ inställningar',
		nl: 'Instellingen resetten',
		pl: 'Przywróć podejście',
		pt_BR: 'Redefinir configurações',
		ru: 'Сбросить настройки',
		zh_CN: '重设置',
		sk: 'Obnoviť nastavenia',
		de: 'Einstellungen zurücksetzen',
		it: 'Impostazioni iniziali',
		es: 'Restablecer configuración',
	},
	'Are you sure ?': {
		fr: 'Etes vous sûr ?',
		sv: 'Är du säker ?',
		nl: 'Weet je zeker dat ?',
		pl: 'Czy jesteś pewien?',
		pt_BR: 'Você tem certeza?',
		ru: 'Вы уверены ?',
		zh_CN: '确定还原设置吗?',
		sk: 'Si si istý ?',
		de: 'Bist du sicher ?',
		it: 'Sei sicuro?',
		es: '¿Estás seguro?',
	},
	'Made in France with ❤️': {
		fr: 'Fabriqué en France avec ❤️',
		sv: 'Tillverkad i frankrike med ❤️',
		nl: 'Gemaakt in Frankrijk ❤️',
		pl: 'Stworzone we Francji z ❤️',
		pt_BR: 'Feito na França com ❤️',
		ru: 'Сделано во франции с ❤️',
		zh_CN: '法国大师❤️全新',
		sk: 'Spravené vo Francúzsku s ❤️',
		de: 'Gemacht in Frankreich mit ❤️',
		it: 'Fatto in Francia con ❤️',
		es: 'Hecho en Francia con ❤️',
	},
	by: {
		fr: 'par',
		sv: 'av',
		nl: 'Deur',
		pl: 'przez',
		pt_BR: 'por',
		ru: 'Разработчики:',
		zh_CN: '由',
		sk: 'od',
		de: 'von',
		it: 'Da',
		es: 'por',
	},
	Visit: {
		fr: 'Rendez-vous sur',
		sv: 'Besök',
		nl: 'Bezoek',
		pl: 'Do zobaczenia na',
		pt_BR: 'Veja',
		ru: 'Посетите',
		zh_CN: '访问',
		sk: 'Navštívte',
		de: 'Besuchen',
		it: 'Visita',
		es: 'Visitar',
	},
	'our website': {
		fr: 'notre site Internet',
		sv: 'vår hemsida',
		nl: 'onze website',
		pl: 'nasza strona',
		pt_BR: 'nosso site',
		ru: 'наш сайт',
		zh_CN: '我们的网站',
		sk: 'naša webstránku',
		de: 'unsere Website',
		it: 'il nostro sito internet',
		es: 'nuestra página web',
	},
	'for infos and donations': {
		fr: 'pour en savoir plus ou faire un don',
		sv: 'för mera information och donationer',
		nl: 'Voor informatie en donaties',
		pl: 'aby dowiedzieć się więcej lub przekazac darowiznę',
		pt_BR: 'para mais informações e doações',
		ru: 'чтобы узнать больше или поддержать разработчиков',
		zh_CN: '以获取更多信息,或向我们采取行动',
		sk: 'pre informácie a dary',
		de: 'für Informationen und Spenden',
		it: 'per info e donazioni',
		es: 'para más información y donaciones',
	},
}

const days = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday']

const months = [
	'January',
	'February',
	'March',
	'April',
	'May',
	'June',
	'July',
	'August',
	'September',
	'October',
	'November',
	'December',
]

const engineLocales = {
	google: {
		base: 'https://www.google%l/search?q=%s',
		fr: '.fr',
		en: '.com',
		ru: '.ru',
		pl: '.pl',
		pt_BR: '.com.br',
		sv: '.se',
		sl: '.sl',
		nl: '.nl',
		zh_CN: '.tw',
		sk: '.sk',
		de: '.de',
		it: '.it',
		es: '.es',
		no: '.no',
	},
	ddg: {
		base: 'https://duckduckgo.com/?q=%s&kl=%l',
		fr: 'fr-fr',
		en: 'en-US',
		ru: 'ru-ru',
		pl: 'pl-pl',
		pt_BR: 'br-pt',
		sv: 'se-se',
		sl: 'sl-sl',
		nl: 'nl-nl',
		zh_CN: 'cn-zh',
		sk: 'sk-sk',
		de: 'de-de',
		it: 'it-it',
		es: 'es-es',
		no: 'no-no',
	},
	startpage: {
		base: 'https://www.startpage.com/do/dsearch?query=%s&language=%l',
		fr: 'francais',
		en: 'english',
		ru: 'russian',
		pl: 'polski',
		pt_BR: 'br-pt',
		sv: 'svenska',
		sl: 'sl-sl',
		nl: 'nl-nl',
		zh_CN: 'cn-zh',
		sk: 'sk-sk',
		de: 'de-de',
		it: 'italiano',
		es: 'es-es',
		no: 'norsk',
	},
	qwant: {
		base: 'https://www.qwant.com/?q=%s&r=%l',
		fr: 'FR&sr=fr',
		en: 'US&sr=en',
		ru: 'RU&sr=ru',
		pl: 'PL&sr=pl',
		pt_BR: 'BR&sr=pt',
		sv: 'SE&sr=sv',
		sl: 'SL&sr=sl',
		nl: 'NL&sr=nl',
		zh_CN: 'SG&sr=en',
		sk: 'en&sr=en',
		de: 'DE&sr=de',
		it: 'IT&sr=it',
		es: 'ES&sr=es',
		no: 'NO&sr=nb',
	},
	yahoo: {
		base: 'https://%lsearch.yahoo.com/search?q=%s',
		fr: 'fr.',
		en: '',
		ru: 'ru.',
		pl: 'pl.',
		pt_BR: 'br.',
		sv: 'se.',
		sl: 'sl.',
		nl: 'nl.',
		zh_CN: '',
		sk: '',
		de: 'de.',
		it: 'it.',
		es: 'es.',
		no: 'no.',
	},
	bing: {
		base: 'https://www.bing.com/search?q=%s&cc=%l',
		fr: 'fr',
		en: '',
		ru: 'ru',
		pl: 'pl',
		pt_BR: 'br',
		sv: 'se',
		sl: 'sl',
		nl: 'nl',
		zh_CN: '',
		sk: 'sk',
		de: 'de',
		it: 'it',
		es: 'es',
		no: 'no',
	},
	brave: {
		base: 'https://search.brave.com/search?q=%s',
	},
	ecosia: {
		base: 'https://www.ecosia.org/search?q=%s',
	},
	lilo: {
		base: 'https://search.lilo.org/?q=%s',
	},
}
