...
function traduction(ofSettings, initStorage) {
	...
	if (langue !== 'en') {
		for (let k = 0; k < trns.length; k++) {
			...
			if (dict[trns[k].innerText]) dom.push(dict[trns[k].innerText][langue])
			else dom.push(trns[k].innerText)
		}
...
function date(event, usdate) {
	...
	if (event) storage('usdate', usdate)
}
...
function quickLinks(event, that, initStorage) {
	...
	function appendblock(arr, index, links) {
		...
		//le DOM du block
		let b = `<div class='block' draggable="false" source='${arr.url}'>
	...
	function request(arg, wCat) {
		...
		request_w.onload = function () {
			...
			if (request_w.status >= 200 && request_w.status < 400) {
				if (wCat === 'current') weatherResponse(arg, resp)
				else if (wCat === 'forecast') forecastResponse(arg, resp)
			} else submissionError(resp.message)
		}
...
function initBackground(initStorage) {
	function loadCustom({ custom, customIndex }) {
		imgBackground(b64toBlobUrl(custom.slice(custom.indexOf(',') + 1, custom.length)))
	...
	} else if (type === 'dynamic' || type === 'default') unsplash(initStorage.dynamic)
	else unsplash(null, null, true) //on startup

	let blur = initStorage.background_blur ? initStorage.background_blur : 15
	let bright = initStorage.background_bright ? initStorage.background_bright : 0.7
